...
PROXY_API_KEY = os.environ.get(
    "ANTHROPIC_API_KEY", ""
)  # Proxy can reuse key or be empty
...
def compare_responses(anthropic_response, proxy_response):
    ...
    assert "content" in proxy_json and isinstance(proxy_json["content"], list), (
        "Proxy content is invalid"
    )
    ...
    try:
        ...
        if not ANTHROPIC_ENABLED:
            if proxy_response and proxy_response.status_code == expected_status:
                print(
                    f"✅ Proxy returned expected status code {expected_status} for provider '{provider}'."
                )
                return True
            else:
                status = proxy_response.status_code if proxy_response else "no response"
                print(
                    f"❌ Proxy returned {status} for '{provider}', expected {expected_status}."
                )
                return False
        ...
        anthropic_response = get_response(
            ANTHROPIC_API_URL, anthropic_headers, request_data
        )
        ...
        print(
            f"Proxy status code: {proxy_response.status_code if proxy_response else 'N/A'}"
        )

        if not (
            anthropic_response and proxy_response and proxy_response.status_code == 200
        ):
            print(
                "❌ Test failed: One or both requests failed or proxy did not return 200."
            )
            return False
...
class StreamStats:
    ...
    def summarize(self):
        ...
        if self.text_content:
            print(f"Text preview: '{self.text_content[:70]}...'")
        else:
                    ...
                    while "\n\n" in buffer:
                        ...
                        if "data: " in event_text:
                            ...
                            try:
                                ...
                                print(
                                    f"Warning: Could not decode JSON from stream chunk: {data_part}"
                                )
...
def compare_stream_stats(anthropic_stats, proxy_stats):
    ...
    if not anthropic_stats:
        if not proxy_has_content:
            ...
            return False
        if not (
            "content_block_delta" in proxy_stats.event_types
            or "message_delta" in proxy_stats.event_types
        ):
            print(
                "❌ Proxy-only test failed: stream did not contain any content delta events."
            )
            return False
    ...
    anthropic_has_content = (
        anthropic_stats.has_tool_use or len(anthropic_stats.text_content) > 0
    )
...
async def test_streaming(test_name, request_data):
    ...
    try:
        ...
        if ANTHROPIC_ENABLED:
            anthropic_stats = await stream_response(
                ANTHROPIC_API_URL, anthropic_headers, request_data, "Anthropic"
            )

        proxy_stats = await stream_response(
        ...
        if not ANTHROPIC_ENABLED:
            ...
            if PROVIDER_KEYS.get(provider) and proxy_stats.has_error:
                print(
                    f"❌ Configured provider '{provider}' should not have errored in stream."
                )
                result = False
...
async def main():
    ...
    parser.add_argument(
        "--proxy-only",
        action="store_true",
        help="Skip calling Anthropic, only test proxy",
    )
    parser.add_argument(
        "--provider",
        type=str,
        help="Run only tests for a specific provider (e.g., openai, gemini)",
    )
